...
typedef struct// to call in program
{
    ...
    char mobileName[MAX_MOBILE_NUMBER];// declare the character data type
...
void welcomeMessage()
{
    headMessage("Developed By Ferdaus Hassan id:221002171");
    printf("\n\n\n\n\n");
    printf("\n\t\t\t  **-**-**-**-**-**-**-**-**-**-**-**-**-**-**-**-**-**-**\n");
    printf("\n\t\t\t        =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=");
    printf("\n\t\t\t        =                  WELCOME                  =");
...
void addStudentInDataBase()
{
    ...
    do
    {
        printf("\n\t\t\tStudent Mobile  = ");
        fflush(stdin);
        fgets(addStudentInfoInDataBase.mobileName,MAX_MOBILE_NUMBER,stdin);
    }
    while(!status);
    do
    {
...
void searchStudent()
{
    ...
    if(found)
    {
        ...
        printf("\n\t\t\tStudent Mobile = %s",addStudentInfoInDataBase.mobileName);
...
void viewStudent()
{
    ...
    while (fread (&addStudentInfoInDataBase, sizeof(addStudentInfoInDataBase), 1, fp))
    {
        ...
        printf("\t\t\tStudent Name = %s",addStudentInfoInDataBase.mobileName);
        printf("\t\t\tFather Name = %s",addStudentInfoInDataBase.fatherName);
...
void init()
{
    FILE *fp = NULL;
    int status = 0;
    const char defaultUsername[] ="Ferdaus";
    const char defaultPassword[] ="505050F";
    sFileHeader fileHeaderInfo = {0};
...
{   //extra
    ...
    SetConsoleTitle("Ferdaus Hassan"); // Here add the title of the window
    //extra
    init();
    welcomeMessage();
    login();
